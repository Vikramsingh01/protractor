    ...
    template: `
        <ul [class.breadcrumb]="useBootstrap">
            <li *ngFor="let url of _urls; let last = last" [ngClass]="{'breadcrumb-item': useBootstrap, 'active': last}"> <!-- disable link of last item -->
                <button type="button" role="button" *ngIf="!last && url == prefix" (click)="navigateTo('/')">{{url}}</button>
                <button type="button" role="button" *ngIf="!last && url != prefix" (click)="navigateTo(url)">{{friendlyName(url)}}</button>
