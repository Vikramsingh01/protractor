import {Component, OnInit, Input, Self, ElementRef, ChangeDetectionStrategy } from '@angular/core';
import { NgClass } from '@angular/common';
import { FormGroup, NgModel, ControlValueAccessor } from '@angular/forms';
...
function addseconds(date:any, seconds:number) {
  seconds=0;
...
@Component({
  ...
  template: `
    <div class="relative">
    <div [formGroup]="form" class="relative ntt">
              <input type="text" id="{{controlName}}" [formControlName]="controlName" placeholder="hh:mm" class="selection form-control">
              <button type="button" (click)="isOpen=!isOpen; updateTemplate()" class="time-div">
                <span class="glyphicon glyphicon-time">time</span>
              </button>
    </div>
        <table [ngClass]="{'show': isOpen, 'hide': !isOpen}" class="time-display">
      <tbody>
          ...
          <td *ngIf="showSeconds"><a (click)="incrementSeconds()" [ngClass]="{disabled: noIncrementSeconds()}" class="btn btn-link"><span class="glyphicon glyphicon-chevron-up"></span></a></td>
        ...
        <tr>
          ...
          <td  *ngIf="showSeconds" class="form-group" [ngClass]="{'has-error': invalidSeconds}">
            <input style="width:50px;" type="text" [(ngModel)]="seconds" value="{{seconds}}" (change)="updateSeconds()" class="form-control text-center" [readonly]="readonlyInput" (blur)="minutesOnBlur($event)" maxlength="2">
        ...
        <tr class="text-center" [ngClass]="{hidden: !showSpinners}">
          ...
          <td *ngIf="showSeconds"><a (click)="decrementSeconds()" [ngClass]="{disabled: noDecrementSeconds()}" class="btn btn-link"><span class="glyphicon glyphicon-chevron-down"></span></a></td>
...
export class Timepicker implements ControlValueAccessor, OnInit {
  ...
  @Input() private secondStep:number;
  ...
  @Input() private showSeconds:boolean=false;
  ...
  constructor(@Self() public cd:NgModel,myElement: ElementRef) {
  ...
  ngOnInit() {
    ...
    this.secondStep = def(this.secondStep, isDefined, timepickerConfig.secondStep);
  ...
  private updateTemplate(keyboardChange?:any) {
    ...
    if(this.showSeconds)
    this.form.controls[this.controlName].setValue(this.hours+":"+this.minutes+":"+this.seconds);
     else
     this.form.controls[this.controlName].setValue(this.hours+":"+this.minutes);
  ...
  private updateHours() {
    ...
    let hours = this.getHoursFromTemplate();
    let minutes = this.getMinutesFromTemplate();
    let seconds= this.getSecondsFromTemplate();

    if (!isDefined(hours) || !isDefined(minutes) || !isDefined(seconds)) {
      // todo: validation?
      // invalidate(true);
    }
  ...
  private updateMinutes() {
    ...
    if (!isDefined(minutes) || !isDefined(hours) || isDefined(seconds)) {
      // todo: validation
      // invalidate(undefined, true);
    }
  ...
  private minutesOnBlur(event:Event) {
    ...
    private secondsOnBlur(event:Event) {
    if (this.readonlyInput) {
      return;
    }
 ...
 private noIncrementSeconds() {
    let incrementedSelected = addseconds(this.selected, this.secondStep);
    return incrementedSelected > this.max ||
      (incrementedSelected < this.selected && incrementedSelected < this.min);
  }

   private noDecrementSeconds() {
    let decrementedSelected = addseconds(this.selected, -this.secondStep);
    return decrementedSelected < this.min ||
      (decrementedSelected > this.selected && decrementedSelected > this.max);

  }

   private addSecondsToSelected(seconds:any) {
    this.selected = addseconds(this.selected, seconds);
    this.refresh();
  }
