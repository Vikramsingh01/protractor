import { Component, Input, Output, EventEmitter, OnChanges, OnInit, SimpleChanges, ElementRef, ViewEncapsulation, Renderer } from "@angular/core";
import { FormGroup } from '@angular/forms';
import { IDate, IMonth, IWeek, IDayLabels, IMonthLabels, IOptions } from "./interfaces/index";
import { LocaleService } from "./services/date-picker.locale.service";
import { ValidatorService } from "./services/date-picker.validator.service";
...
export class DatePicker implements OnChanges, OnInit {
    ...
    opts: IOptions = {
        ...
        showDateFormatPlaceholder: <boolean>true,
        customPlaceholderTxt: <string>"dd/mm/yyyy",
    ...
    constructor(private listService: ListService, public elem: ElementRef, private renderer: Renderer, private localeService: LocaleService, private validatorService: ValidatorService) {
    ...
    changeValue(selectedValue) {
        if (this.isNsrd && selectedValue.match(/^(0?[1-9]|[12][0-9]|3[01])[\/\-](0?[1-9]|1[012])[\/\-]\d{4}$/) && selectedValue !== "") {
            let formObj = this.form.value;
            formObj[this.controlName] = selectedValue;
            this.listService.getNSRDDataForChange(formObj).subscribe((data: any) => {
                this.updateNsrd.emit(data.resultMap.fieldObjectList);
            })
        }
        if (this.hasChildAnswers && selectedValue.match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})$/) && selectedValue !== "") {
    ...
    userDateInput(event: any): void {
        ...
        if (event.target.value.length === 0) {
            ...
            let date: IDate = this.validatorService.isDateValid(event.target.value, this.opts.dateFormat, this.opts.minYear, this.opts.maxYear, this.opts.disableUntil, this.opts.disableSince, this.opts.disableWeekends, this.opts.disableDays, this.opts.monthLabels);
    ...
    openBtnClicked(): void {
        if(!this.disabled && !this.form.controls[this.controlName].disabled){
    ...
    clearDate(): void {
        ...
        this.selectionDayTxt = "";
        this.form.controls[this.controlName].setValue(this.selectionDayTxt);
        this.selectedDate = { year: 0, month: 0, day: 0 };
    ...
    selectDate(date: any): void {
        ...
        this.invalidDate = false;
        this.changeValue(this.selectionDayTxt);
