...
export class ContactDetailComponent implements OnInit {
  ...
  private is_external_user: boolean = false;
   private teamlist=[];
    private officerlist=[];
       ...
       //this.authorizationService.getAuthorizationDataByTableId(ContactConstants.tableId).subscribe(authorizationData => {
        ...
        if (this.authorizedFlag) {
            this.contactService.getContact(this.contactId).subscribe(data => {
                this.date = data.modifiedDate.split(" ")[0];
                this.time = data.modifiedDate.split(" ")[1];
                this.finalTime = this.time.split(":")[0];
                this.time = this.time.split(":")[1];
                this.finalTime = this.finalTime + ":" +this.time.split(":")[0];
                if(data.updatedByExternalOrg == "true"){
                    this.is_external_user = true;
                }
                this.contact = data;
            })
        } else {
