...
@Component({
  selector: 'tr-offenderprofile',
  templateUrl: 'offenderprofile.component.html',
...
export class OffenderProfileComponent implements OnInit {
  ...
  ngOnInit() {
  this._titleService.setTitle('My Service Users');
  ...
  //   this.listService.getListData(1).subscribe(teamList=>{
  //     this.teamList = teamList
  // });
  //   this.listService.getListData(1).subscribe(officerList=>{
  //     this.officerList = officerList
  // });

	let offenderProfile: OffenderProfile = new OffenderProfile();
  //offenderProfile.transferPendingYesNoId = 0;
		this.route.params.subscribe((params: any)=>{
			if(params.hasOwnProperty('profileId')) {
				offenderProfile = params['profileId'];
        
			}
		});
    let sortFields: any[] = [];
    sortFields.push({field: "familyName", sort: "asc" }, {field: "firstName", sort: "asc" });
    // this.sortFilterPaginationObj.sortObj.field = "familyName";
    // this.sortFilterPaginationObj.sortObj.sort = "asc";
	...
	}).flatMap(data=>this.offenderProfileService.sortFilterAndPaginate(offenderProfile, this.sortFilterPaginationObj.paginationObj, sortFields)).subscribe(data=>{
  ...
  // searchServiceUser(firstName, familyName, CRN, PNC){
  //   let offenderprofile: OffenderProfile = new OffenderProfile();
  //   if(firstName != "")    offenderprofile.firstName = firstName;
  //   if(familyName != "")    offenderprofile.familyName = familyName;
  //   if(CRN != "")    offenderprofile.caseReferenceNumber = CRN;
  //   if(PNC != "")    offenderprofile.pncNumber = PNC;

  //   offenderprofile.transferPendingYesNoId = 0;
  //   this.offenderProfileService.searchOffenderProfile(offenderprofile).subscribe((data:any)=>{
  ...
  reset(){
    ...
    this.offenderProfileService.sortFilterAndPaginate(offenderProfile, this.sortFilterPaginationObj.paginationObj, sortFields).subscribe((data: any)=>{
      this.offenderProfiles = data.content;
      this.sortFilterPaginationObj.paginationObj = data;
      this.sortFilterPaginationObj.filterObj = offenderProfile;
    ...
    searchOffenderProfile() {
      ...
      this.sortFilterPaginationObj.paginationObj.number=0;
      sortFields.push({field: "familyName", sort: "asc" }, {field: "firstName", sort: "asc" });
      this.offenderProfileService.sortFilterAndPaginate(this.offenderProfileFilterForm.value,this.sortFilterPaginationObj.paginationObj, sortFields).subscribe((data: any) => {
  ...
  sortFilterAndPaginate(filterObj, paginationObj, sortObj){
    ...
    this.offenderProfileService.sortFilterAndPaginate(filterObj, paginationObj, sortObj).subscribe((data: any)=>{
      this.offenderProfiles = data.content;
      this.sortFilterPaginationObj.paginationObj = data;
    });
  }
